object F_parcelas: TF_parcelas
  Left = 125
  Top = 21
  Width = 1126
  Height = 697
  Caption = 'Lan'#231'amento de Parcelas'
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'MS Sans Serif'
  Font.Style = []
  OldCreateOrder = False
  OnActivate = FormActivate
  OnDeactivate = FormDeactivate
  OnShow = FormShow
  PixelsPerInch = 96
  TextHeight = 13
  object PaginaControle_parcelas: TPageControl
    Left = 0
    Top = 0
    Width = 1110
    Height = 658
    ActivePage = Tab_cadastro_parcela
    Align = alClient
    TabOrder = 0
    object Tab_cadastro_parcela: TTabSheet
      Caption = 'Venda'
      object L_cd_produto: TLabel
        Left = 448
        Top = 56
        Width = 37
        Height = 13
        Caption = 'Produto'
      end
      object L_cd_cliente: TLabel
        Left = 24
        Top = 56
        Width = 32
        Height = 13
        Caption = 'Cliente'
      end
      object L_cd_nf: TLabel
        Left = 24
        Top = 16
        Width = 53
        Height = 13
        Caption = 'Nota Fiscal'
        FocusControl = DBE_cd_nf
      end
      object L_cd_serie: TLabel
        Left = 24
        Top = 96
        Width = 24
        Height = 13
        Caption = 'Serie'
        FocusControl = DBE_cd_serie
      end
      object L_vl_total: TLabel
        Left = 448
        Top = 152
        Width = 51
        Height = 13
        Caption = 'Valor Total'
        FocusControl = DBE_vl_total
      end
      object L_in_excluida: TLabel
        Left = 24
        Top = 176
        Width = 40
        Height = 13
        Caption = 'Excluido'
      end
      object L_dt_venda: TLabel
        Left = 448
        Top = 16
        Width = 57
        Height = 13
        Caption = 'Data Venda'
        FocusControl = DBE_dt_venda
      end
      object L_qt_venda: TLabel
        Left = 448
        Top = 104
        Width = 55
        Height = 13
        Caption = 'Quantidade'
        FocusControl = DBE_qt_venda
      end
      object L_vl_venda: TLabel
        Left = 24
        Top = 136
        Width = 58
        Height = 13
        Caption = 'Valor Venda'
        FocusControl = DBE_vl_venda
      end
      object G_progresso: TGauge
        Left = 0
        Top = 304
        Width = 1097
        Height = 25
        Enabled = False
        Progress = 0
        Visible = False
      end
      object B_novo: TBitBtn
        Left = 40
        Top = 219
        Width = 97
        Height = 33
        Caption = 'Novo'
        TabOrder = 0
        OnClick = B_novoClick
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      end
      object B_alterar: TBitBtn
        Left = 248
        Top = 219
        Width = 91
        Height = 33
        Caption = 'Alterar'
        TabOrder = 1
        OnClick = B_alterarClick
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FF2538CC0E1BB8FF00FFFF00FF}
      end
      object B_gravar: TBitBtn
        Left = 456
        Top = 219
        Width = 91
        Height = 33
        Caption = 'Gravar'
        TabOrder = 2
        OnClick = B_gravarClick
        Glyph.Data = {
          
          00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      end
      object B_excluir: TBitBtn
        Left = 664
        Top = 219
        Width = 79
        Height = 33
        Caption = 'Excluir'
        TabOrder = 3
        OnClick = B_excluirClick
        Glyph.Data = {
          
          00FFFF00FFFF00FFFBCE9FFBCE9FFBCE9FFBCE9FFF00FFFF00FF}
      end
      object B_cancel: TBitBtn
        Left = 872
        Top = 219
        Width = 89
        Height = 33
        Caption = 'Cancelar'
        TabOrder = 4
        OnClick = B_cancelClick
        Glyph.Data = {
          
          209AFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FFFF00FF}
      end
      object CB_cd_produto: TDBLookupComboBox
        Left = 448
        Top = 72
        Width = 249
        Height = 21
        DataField = 'CD_PRODUTO'
        DataSource = DS_cadastro_venda_produto
        KeyField = 'CD_PRODUTO'
        ListField = 'DS_PRODUTO'
        ListSource = DS_lista_produto_parcela
        TabOrder = 5
      end
      object CB_cd_cliente: TDBLookupComboBox
        Left = 24
        Top = 72
        Width = 249
        Height = 21
        DataField = 'CD_CLIENTE'
        DataSource = DS_cadastro_venda
        KeyField = 'CD_CLIENTE'
        ListField = 'DS_CLIENTE'
        ListSource = DS_lista_cliente_parcela
        TabOrder = 6
      end
      object DBG_cadastro_parcelas: TDBGrid
        Left = 0
        Top = 336
        Width = 1097
        Height = 305
        DataSource = DS_cadastro_parcelas
        PopupMenu = PopupMenu_parceas
        TabOrder = 7
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        Columns = <
          item
            Expanded = False
            FieldName = 'NR_SEQUENCIA'
            Title.Caption = 'Sequencia'
            Width = 95
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CD_PRODUTO'
            Title.Caption = 'Produto'
            Width = 106
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CD_NF'
            Title.Caption = 'Nota Fiscal'
            Width = 112
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CD_CLIENTE'
            Title.Caption = 'Cliente'
            Width = 118
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CD_SERIE'
            Title.Caption = 'Serie'
            Width = 80
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VL_PARCELA'
            Title.Caption = 'Valor Parcela'
            Width = 154
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VL_PAGO'
            Title.Caption = 'Valor Pago'
            Width = 134
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DT_PAGO'
            Title.Caption = 'Data do Pagamento'
            Width = 121
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CD_REPARCELA'
            Title.Caption = 'Sequencia Reparcela'
            Width = 126
            Visible = True
          end>
      end
      object DBE_cd_nf: TDBEdit
        Left = 24
        Top = 32
        Width = 249
        Height = 21
        DataField = 'CD_NF'
        DataSource = DS_cadastro_venda
        Enabled = False
        TabOrder = 8
      end
      object DBE_cd_serie: TDBEdit
        Left = 24
        Top = 112
        Width = 249
        Height = 21
        DataField = 'CD_SERIE'
        DataSource = DS_cadastro_venda
        TabOrder = 9
      end
      object DBE_vl_total: TDBEdit
        Left = 448
        Top = 168
        Width = 251
        Height = 21
        DataField = 'VL_TOTAL'
        DataSource = DS_cadastro_venda
        Enabled = False
        TabOrder = 10
      end
      object DBE_dt_venda: TDBEdit
        Left = 448
        Top = 32
        Width = 249
        Height = 21
        DataField = 'DT_VENDA'
        DataSource = DS_cadastro_venda
        Enabled = False
        MaxLength = 10
        TabOrder = 11
      end
      object CB_in_excluida: TDBComboBox
        Left = 24
        Top = 192
        Width = 49
        Height = 21
        DataField = 'IN_EXCLUIDA'
        DataSource = DS_cadastro_venda
        ItemHeight = 13
        Items.Strings = (
          'S'
          'N')
        TabOrder = 12
      end
      object DBE_qt_venda: TDBEdit
        Left = 448
        Top = 120
        Width = 249
        Height = 21
        DataField = 'QT_VENDA'
        DataSource = DS_cadastro_venda_produto
        TabOrder = 13
      end
      object DBE_vl_venda: TDBEdit
        Left = 24
        Top = 152
        Width = 249
        Height = 21
        DataField = 'VL_VENDA'
        DataSource = DS_cadastro_venda_produto
        TabOrder = 14
      end
      object CB_quantidade_parcelas: TComboBox
        Left = 160
        Top = 272
        Width = 145
        Height = 21
        Enabled = False
        ItemHeight = 13
        TabOrder = 15
        Text = 'Quantas Parcelas?'
        Items.Strings = (
          '1'
          '2'
          '3'
          '4'
          '5'
          '6'
          '7'
          '10'
          '15'
          '20'
          '30')
      end
      object B_gerar_parcelas: TButton
        Left = 536
        Top = 272
        Width = 169
        Height = 25
        Caption = 'Gerar Parcelas'
        Enabled = False
        TabOrder = 16
        OnClick = B_gerar_parcelasClick
      end
      object cb_distancia_parcelas: TComboBox
        Left = 328
        Top = 272
        Width = 185
        Height = 21
        Enabled = False
        ItemHeight = 13
        TabOrder = 17
        Text = 'Distancia em dias entre parcelas'
        Items.Strings = (
          '15'
          '20'
          '30'
          '45'
          '60')
      end
    end
    object Tab_consulta_parcela: TTabSheet
      Caption = 'Consultas'
      ImageIndex = 1
      object DBG_consulta_parcelas: TDBGrid
        Left = 0
        Top = 56
        Width = 1073
        Height = 265
        DataSource = DS_consulta_parcelas
        Options = [dgTitles, dgIndicator, dgColumnResize, dgColLines, dgRowLines, dgTabs, dgRowSelect, dgAlwaysShowSelection, dgConfirmDelete, dgCancelOnExit]
        TabOrder = 0
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        OnDblClick = DBG_consulta_parcelasDblClick
        Columns = <
          item
            Expanded = False
            FieldName = 'NR_SEQUENCIA'
            Title.Caption = 'Sequencia'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CD_NF'
            Title.Caption = 'Nota Fiscal'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CD_SERIE'
            Title.Caption = 'Serie'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VL_PARCELA'
            Title.Caption = 'Valor Parcela'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VL_PAGO'
            Title.Caption = 'Valor Pago'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DT_PAGO'
            Title.Caption = 'Data'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DS_CLIENTE'
            Title.Caption = 'Cliente'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DS_PRODUTO'
            Title.Caption = 'Produto'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CD_REPARCELA'
            Title.Caption = 'Reparcela'
            Visible = True
          end>
      end
      object E_pesquisar_parcela: TEdit
        Left = 0
        Top = 16
        Width = 329
        Height = 21
        TabOrder = 1
        Text = 'Pesquisar Parcelas'
      end
      object B_pesquisar_parcela: TButton
        Left = 344
        Top = 16
        Width = 105
        Height = 25
        Caption = 'Pesquisar'
        TabOrder = 2
        OnClick = B_pesquisar_parcelaClick
      end
      object RG_parcelas: TRadioGroup
        Left = 464
        Top = 8
        Width = 425
        Height = 41
        Caption = 'Selecione a Pesquisa:'
        Columns = 4
        Items.Strings = (
          'Sequencia'
          'Nota Fiscal'
          'Cliente'
          'Produto')
        TabOrder = 3
      end
      object DBG_consulta_venda: TDBGrid
        Left = 0
        Top = 360
        Width = 1073
        Height = 265
        DataSource = DS_consulta_venda
        TabOrder = 4
        TitleFont.Charset = DEFAULT_CHARSET
        TitleFont.Color = clWindowText
        TitleFont.Height = -11
        TitleFont.Name = 'MS Sans Serif'
        TitleFont.Style = []
        OnDblClick = DBG_consulta_vendaDblClick
        Columns = <
          item
            Expanded = False
            FieldName = 'CD_NF'
            Title.Caption = 'Nota Fiscal'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DS_CLIENTE'
            Title.Caption = 'Cliente'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'CD_SERIE'
            Title.Caption = 'Serie'
            Width = 45
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DT_VENDA'
            Title.Caption = 'Data'
            Width = 88
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'DS_PRODUTO'
            Title.Caption = 'Produto'
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'QT_VENDA'
            Title.Caption = 'Quantidade'
            Width = 78
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VL_VENDA'
            Title.Caption = 'Valor'
            Width = 77
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'VL_TOTAL'
            Title.Caption = 'Valor Total'
            Width = 138
            Visible = True
          end
          item
            Expanded = False
            FieldName = 'IN_EXCLUIDA'
            Title.Caption = 'Excluido'
            Visible = True
          end>
      end
      object E_pesquisar_venda: TEdit
        Left = 0
        Top = 328
        Width = 329
        Height = 21
        TabOrder = 5
        Text = 'Pesquisar Vendas'
      end
      object B_pesquiar_venda: TButton
        Left = 336
        Top = 328
        Width = 121
        Height = 25
        Caption = 'Pesquisar'
        TabOrder = 6
        OnClick = B_pesquiar_vendaClick
      end
    end
  end
  object DS_cadastro_parcelas: TDataSource
    DataSet = DataModulo.IBD_parcelas
    Left = 904
    Top = 24
  end
  object DS_lista_produto_parcela: TDataSource
    DataSet = DataModulo.IBQ_lista_produto_parcelas
    Left = 948
    Top = 24
  end
  object DS_lista_cliente_parcela: TDataSource
    DataSet = DataModulo.IBQ_lista_cliente_parcela
    Left = 996
    Top = 24
  end
  object DS_consulta_parcelas: TDataSource
    DataSet = DataModulo.IBQ_parcelas
    Left = 1044
    Top = 24
  end
  object DS_cadastro_venda: TDataSource
    DataSet = DataModulo.IBD_venda
    OnStateChange = DS_cadastro_vendaStateChange
    Left = 904
    Top = 64
  end
  object DS_cadastro_venda_produto: TDataSource
    DataSet = DataModulo.IBD_venda_produto
    OnStateChange = DS_cadastro_venda_produtoStateChange
    Left = 904
    Top = 104
  end
  object PopupMenu_parceas: TPopupMenu
    Left = 612
    Top = 448
    object PagarParcela1: TMenuItem
      Caption = 'Pagar Parcela'
      OnClick = PagarParcela1Click
    end
    object PagarParcial1: TMenuItem
      Caption = 'Pagar Parcial'
      OnClick = PagarParcial1Click
    end
    object PagarTotal1: TMenuItem
      Caption = 'Pagar Total'
      OnClick = PagarTotal1Click
    end
    object ExcluirParcela1: TMenuItem
      Caption = 'Excluir Parcela'
      OnClick = ExcluirParcela1Click
    end
  end
  object DS_consulta_venda: TDataSource
    DataSet = DataModulo.IBQ_venda
    Left = 1044
    Top = 64
  end
end
