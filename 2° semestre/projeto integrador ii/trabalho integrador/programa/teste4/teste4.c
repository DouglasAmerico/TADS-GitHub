...
//---------------
  ...
  {
      char cor[10];
      int num_registro_cor_tipo_de_flor;
  } cor_de_flor;
//-------------------------------
    ...
    arq_t_flores= fopen ("Tipo_de_flores.txt","r+b");
     fseek (arq_t_flores,0, SEEK_END);
       do
       {
           ...
           gets   (tipo.cor_flor.cor);
           fflush (stdin);
           tipo.registro= tipo.registro + 1;
           tipo.cor_flor.num_registro_cor_tipo_de_flor++; // numero de registro adicionado altomaticamente
...
// Verificações
//-------------------------------
 void revisao_existe_t_flores(int num_registro,int *contem)
 {
       int existe=0;
       arq_t_flores= fopen ("Tipo_de_flores.txt","r+b");
       while (fread(&tipo,sizeof(tipo),1,arq_t_flores))
                      ...
                      if (num_registro == tipo.registro)
                       {
                         if (tipo.ativo == 0)
                          {
                              if (strcmpi(cor_requerida,tipo.cor)== 0)
                              {
                                existe= 0; // variavel de localização ativa
                                printf ("Nome: %s\n",tipo.nome);
                                printf ("Numero de registro: %d\n",tipo.registro);
     ...
     fseek (arq_fornecedores,0, SEEK_END);
       do
       {
           ...
           else
           {
            ...
            printf ("Escolha qual opção de tipo de flor:\n");
            visualizar_tipo_de_flores_geral();
            printf ("Informe o numero de registro do tipo de flor oferecidas pelo %s: ",pessoa.nome);
            fflush (stdin);
            scanf  ("%d",&pessoa.num_registro_tipo_de_flor_oferecida);
            fflush (stdin);
            revisao_existe_t_flores(pessoa.num_registro_tipo_de_flor_oferecida,&contem);//verificação da existencia do tipo de flor digitado
            if (contem == 1)
            {
                system ("cls");
